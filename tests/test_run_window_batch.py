...
class TestTheQueueIsSummed:
    ...
    def test_it_is_not_priced_as_the_longest_zone(self):
        ...
        """
        runs = [_run(0, 600), _run(1, 900), _run(2, 300)]
        assert _clock(runs) != pytest.approx(900)
...
class TestPausesAreNotPriced:
    ...
    def test_a_configured_pause_timeout_does_not_change_the_price(self):
        """Nothing about pricing reads the pause configuration at all."""
        runs = [_run(0, 600), _run(1, 600)]
        before = _clock(runs)
        assert const.CONF_BATCH_PAUSE_TIMEOUT  # the setting exists...
        assert _clock(runs) == before  # ...and the window does not consult it.


class TestDatetimeIsNotNeeded:
    """The module stays pure arithmetic; the batch track adds no clock reads."""

    def test_pricing_is_a_function_of_its_arguments(self):
        runs = [_run(0, 600), _run(1, 900)]
        first = _clock(runs)
        second = _clock(runs)
        assert first == second == pytest.approx(1500)
        assert isinstance(datetime.timedelta(seconds=first), datetime.timedelta)
