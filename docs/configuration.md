...
- **Setup** — everything you configure once and rarely touch. The tabs are grouped around what you're trying to do rather than the internal data model, and the docs follow the same structure:
  ...
  5. [**Experimental**](configuration-experimental.md) — opt-in features that change how the bucket is filled (forecast-weighted durations, crediting the bucket from observed watering). Off by default.
