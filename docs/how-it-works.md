...
This estimate never changes the stored `bucket`, and it is anchored to the last calculation so it never double-counts the evapotranspiration the daily calculation has already applied. By default it is also **display-only**: irrigation is triggered and sized from the stored `bucket` alone. That changes if you turn on [live-estimate watering](configuration-experimental.md#live-estimate-watering) — each scheduled run then both triggers and sizes itself from this estimate, and the dashboard's watering decision shows the duration that run will use.
