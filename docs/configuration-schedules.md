...
> **How the run length is estimated per watering mode.** Zones in *Classic* mode are combined according to the [zone sequencing](configuration-when-to-water.md#zone-sequencing) setting. Zones on *Self-closing service* valves are assumed to open together, because the hardware owns each close. *OpenSprinkler station* zones are assumed to run one after another: whether two stations run at once is a per-station **sequential group** in the controller's own configuration that Home Assistant cannot read, so the window reserves enough time for the longer case. A controller whose stations really are in a parallel group therefore finishes early — the finish time is never overshot. The three groups are started together, so the run is as long as the longest of them rather than their total.
