...
class SmartIrrigationCoordinator(DataUpdateCoordinator):
    ...
    async def _irrigate_zones_rotating(self, zones: list):
        ...
        """
        max_slot = max(1, (self.store.config.zone_sequencing_max_consecutive_duration or 5)) * 60
        min_absorption = (self.store.config.zone_sequencing_min_absorption_time or 0) * 60
        ...
        # --- Timed zone state ---
        timed_remaining = {z[const.ZONE_ID]: float(z[const.ZONE_DURATION]) for z in timed_zones}
        timed_by_id = {z[const.ZONE_ID]: z for z in timed_zones}

        # --- Flow meter zone state ---
        ha_metric = self.hass.config.units is METRIC_SYSTEM
        flow_target: dict = {}    # zone_id -> target volume (L)
        flow_delivered: dict = {} # zone_id -> total volume delivered (L)
        flow_elapsed: dict = {}   # zone_id -> total planned open-time (s), for safety cap
        flow_size_m2: dict = {}   # zone_id -> size in m² (for litre→mm conversion)
        flow_bucket: dict = {}    # zone_id -> current bucket in native units (live-updated)
        flow_by_id: dict = {}
        ...
        for z in flow_zones:
            ...
            flow_bucket[zid] = raw_bucket  # native unit, updated after each slot
            flow_by_id[zid] = z
            _LOGGER.info(
        ...
        def _flow_done(zid):
            ...
            return flow_delivered[zid] >= flow_target[zid] or flow_elapsed[zid] >= safety
        ...
        while not _all_done():
            for zid in zone_order:
                ...
                # Enforce absorption time before re-irrigating this zone
                ...
                if is_flow:
                    ...
                    delivered = await self._irrigate_zone_flow_slot(z, entity_id, slot, rem_vol)
                    ...
                    # Update bucket immediately after each slot
                    if flow_size_m2[zid] > 0:
                        slot_mm = delivered / flow_size_m2[zid]
                        slot_native = (
                            slot_mm
                            if ha_metric
                            else convert_between(const.UNIT_MM, const.UNIT_INCH, slot_mm)
                        )
                        prev_bucket = flow_bucket[zid]
                        new_bucket = min(0.0, prev_bucket + slot_native)
                        flow_bucket[zid] = new_bucket
                        await self.store.async_update_zone(zid, {const.ZONE_BUCKET: new_bucket})
                        _LOGGER.info(
